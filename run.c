...
char* format_to_llama_chat_style(char *prompt) {
    char* BOS =  "<s>";
    char* EOS = "</s>";
    char* B_INST = "[INST] ";
    char* E_INST = " [/INST]";
    ...
    char* DEFAULT_SYSTEM_PROMPT = "You are a helpful assistant. If you don't know the answer to a question, please don't share false information.";

    size_t total_length = strlen(B_INST) + strlen(E_INST) + strlen(prompt) + strlen(B_SYS) + strlen(E_SYS) + strlen(DEFAULT_SYSTEM_PROMPT) + 1;

    char* new_prompt = (char*)malloc(total_length * sizeof(char));

    strcpy(new_prompt, B_INST);
    strcat(new_prompt, B_SYS);
    strcat(new_prompt, DEFAULT_SYSTEM_PROMPT);
    strcat(new_prompt, E_SYS);
    strcat(new_prompt, prompt);
    strcat(new_prompt, E_INST);
    new_prompt[total_length] = '\0';
...
int main(int argc, char *argv[]) {
    ...
    float temperature = 0.6f; // e.g. 1.0, or 0.0
    ...
    while (1) {
        ...
        char* prompt;
        size_t size = 0;
        ssize_t chars_read = getline(&prompt, &size, stdin);  
        // if (round == 0) 
        prompt = format_to_llama_chat_style(prompt);
        ...
        // printf("token:%d\n", token);
        // printf("%d\n", prompt_tokens[num_prompt_tokens-2]);
